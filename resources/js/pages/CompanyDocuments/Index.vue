...
import CompanyDocumentUploadModal from '@/components/CompanyDocumentUploadModal.vue';
...
interface Props {
    can_manage_company_documents: boolean;
}
...
const categoryOptions = computed(() => {
    ...
    if (props.can_manage_company_documents) {
...
/**
 * Fetch company documents from API.
 */
async function fetchDocuments() {
    ...
    try {
        let url = `/api/company-documents?page=${currentPage.value}`;
...
async function handleDelete(document: Document) {
    try {
        const url = `/api/company-documents/${document.id}`;
...
<template>
    <Head :title="`Company Documents - ${tenantName}`" />

    <TenantLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6" data-test="company-documents-page">
            <!-- Page Header -->
            <div
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        Manage company-wide documents, policies, and handbooks
                        accessible to all employees.
                    </p>
                </div>
                <Button
                    v-if="props.can_manage_company_documents"
            ...
            <div
                ...
                class="flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-white py-12 text-center dark:border-slate-700 dark:bg-slate-900"
            ...
            >
                ...
                <Button
                    v-if="
                        props.can_manage_company_documents &&
                        !selectedCategoryId
                    "
                    class="mt-4"
                    @click="handleUploadClick"
            ...
            <template v-else>
                    ...
                    >
                        ...
                        >
                            <DocumentTableRow
                                ...
                                :key="document.id"
                                :document="document"
                                :can-manage="props.can_manage_company_documents"
                ...
                <div
                    ...
                    class="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-3 sm:px-6 dark:border-slate-700 dark:bg-slate-900"
