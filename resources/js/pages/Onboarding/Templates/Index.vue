...
function toggleActive(template: Template) {
    ...
    router.post(`/api/onboarding-templates/${template.id}/toggle-active`, {}, {
        preserveState: true,
        onFinish: () => {
...
function deleteTemplate(template: Template) {
    ...
    router.delete(`/api/onboarding-templates/${template.id}`, {
        preserveState: true,
        onSuccess: () => {
            router.reload({ only: ['templates'] });
        },
...
<template>
    <Head :title="`Onboarding Templates - ${tenantName}`" />
    ...
    <TenantLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-4xl">
            ...
            <div v-if="templates.length === 0" class="rounded-lg border border-slate-200 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-800">
                ...
                <p class="mt-3 text-slate-500 dark:text-slate-400">No templates yet. Create your first onboarding template.</p>
                <Link href="/onboarding-templates/create" class="mt-4 inline-block">
                    <Button>
                        <Plus class="mr-1.5 h-4 w-4" />
                        Create Template
                    </Button>
                ...
                <Card v-for="template in templates" :key="template.id">
                    <CardContent class="flex items-center justify-between pt-6">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            >
                                <Check v-if="!template.is_active" class="mr-1.5 h-4 w-4" />
                                <X v-else class="mr-1.5 h-4 w-4" />
                                {{ template.is_active ? 'Deactivate' : 'Activate' }}
                            </Button>
                            <Link :href="`/onboarding-templates/${template.id}/edit`">
