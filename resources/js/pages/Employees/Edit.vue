...
const form = useForm({
    ...
    department_id: props.employee.department_id?.toString() || '',
    position_id: props.employee.position_id?.toString() || '',
    work_location_id: props.employee.work_location_id?.toString() || '',
    supervisor_id: props.employee.supervisor_id?.toString() || '',
    ...
    nbi_clearance: props.employee.nbi_clearance || '',
    police_clearance: props.employee.police_clearance || '',
    prc_license: props.employee.prc_license || '',
...
const supervisorOptions = computed<EnumOption[]>(() => {
    return props.employees
        .filter((emp) => emp.id !== props.employee.id)
        .map((emp) => ({
...
function handleSubmit() {
    form.transform((data) => ({
        ...
        work_location_id: data.work_location_id
            ? parseInt(data.work_location_id)
            : null,
        supervisor_id: data.supervisor_id ? parseInt(data.supervisor_id) : null,
...
function handleCancel() {
    router.visit(`/employees/${props.employee.id}`);
}
</script>

<template>
    <Head :title="`Edit ${employee.full_name} - ${tenantName}`" />
