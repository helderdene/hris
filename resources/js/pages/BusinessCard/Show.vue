...
const formattedAddress = computed(() => {
    const loc = props.employee.work_location;
    if (!loc) return null;
    return [loc.address, loc.city, loc.region, loc.country]
        .filter(Boolean)
        .join(', ');
});

const hasSocialLinks = computed(() => {
    return props.company.linkedin || props.company.facebook || props.company.instagram || props.company.twitter;
});
        ...
        >
            ...
            <div class="space-y-3 px-6 pt-4 pb-2">
                ...
                <div
                    ...
                    class="flex items-center gap-3 rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-700 dark:bg-slate-700/50 dark:text-slate-300"
                >
                    <svg
                        class="h-5 w-5 shrink-0 text-slate-400"
                        xmlns="http://www.w3.org/2000/svg"
            ...
            <div
                v-if="hasSocialLinks"
                class="flex justify-center gap-3 px-6 pt-2 pb-2"
            >
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-500 transition-colors hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600"
                    title="LinkedIn"
                >
