...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Reports', href: '/reports/pagibig' },
    { title: 'Pag-IBIG Reports', href: '/reports/pagibig' },
];
...
const canPreview = computed(() => {
    return selectedReportType.value && selectedYear.value && selectedMonth.value;
});

// Watch for changes and fetch preview
watch([selectedReportType, selectedYear, selectedMonth], () => {
    if (canPreview.value) {
        fetchPreview();
    }
});
...
async function fetchPreview() {
    ...
    try {
        const response = await fetch('/api/reports/pagibig/preview', {
            method: 'POST',
            headers: {
...
async function generateReport(format: 'xlsx' | 'pdf' | 'csv') {
    ...
    try {
        const response = await fetch('/api/reports/pagibig/generate', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!filename) {
            filename = `pagibig_${selectedReportType.value}_${selectedYear.value}-${String(selectedMonth.value).padStart(2, '0')}.${format}`;
        }
...
<template>
    <Head :title="`Pag-IBIG Reports - ${tenantName}`" />
        ...
        <div class="flex flex-col gap-6">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Generate Pag-IBIG compliance reports for regulatory submission.
                </p>
            </div>
                        ...
                        >
                            <thead class="bg-slate-50 dark:bg-slate-800/50">
                                <tr>
                                    ...
                                    <template v-if="selectedReportType === 'mcrf'">
                                        <th
                                    ...
                                    <template
                                        v-else-if="
                                            selectedReportType === 'stl' ||
                                            selectedReportType === 'hdl'
                                        "
                                    >
                                        <th
                                            v-if="selectedReportType === 'stl'"
                                            class="px-4 py-3 text-left text-xs font-medium tracking-wider text-slate-500 uppercase dark:text-slate-400"
                                        >
                                            Loan Type
                                        </th>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <template v-if="selectedReportType === 'mcrf'">
                                        ...
                                        >
                                            {{
                                                formatCurrency(row.pagibig_employee)
                                            }}
                                        </td>
                                        <td
                                            class="px-4 py-3 text-right text-sm text-slate-600 whitespace-nowrap dark:text-slate-400"
                                        >
                                            {{
                                                formatCurrency(row.pagibig_employer)
                                            }}
                                        </td>
                                        <td
                                ...
                                <tr>
                                    ...
                                    <template v-if="selectedReportType === 'mcrf'">
                                        ...
                                        >
                                            {{
                                                formatCurrency(
                                                    previewTotals.pagibig_employee,
                                                )
                                            }}
                                        </td>
                                        <td
                                        ...
                                        >
                                            {{
                                                formatCurrency(
                                                    previewTotals.pagibig_employer,
                                                )
                                            }}
                                        </td>
                                        <td
