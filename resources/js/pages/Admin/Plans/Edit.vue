...
const form = useForm({
    ...
    tenant_id: props.plan.tenant_id ?? ('' as string | number),
    modules: [...props.plan.modules],
    limits: {
        max_employees: (props.plan.limits?.max_employees as number) ?? 50,
        max_admin_users: (props.plan.limits?.max_admin_users as number) ?? 5,
        max_departments: (props.plan.limits?.max_departments as number) ?? 10,
        max_biometric_devices: (props.plan.limits?.max_biometric_devices as number) ?? 5,
        storage_gb: (props.plan.limits?.storage_gb as number) ?? 10,
        api_access: (props.plan.limits?.api_access as boolean) ?? false,
    },
    prices: props.plan.prices.map((p) => ({
...
function submit() {
    form.put(`/admin/plans/custom/${props.plan.id}`, {
        preserveScroll: true,
    });
}
...
<template>
    <AdminLayout
        :breadcrumbs="[
            ...
            { title: `Edit: ${plan.name}`, href: `/admin/plans/custom/${plan.id}/edit` },
        ]"
    >
