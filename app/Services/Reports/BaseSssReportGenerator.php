...
/**
 * Base class for SSS report generators.
 *
 * Provides common functionality for data retrieval and format conversion.
 */
abstract class BaseSssReportGenerator
{
    /**
    ...
    /**
     * Get the report code for filenames (e.g., 'r3', 'r5', 'sbr', 'ecl').
     */
    ...
    public function toExcel(array $data, int $year, ?int $month, ?int $quarter): array
    {
        ...
        $sssNumber = $tenant?->business_info['sss_number'] ?? '';
        ...
        if ($sssNumber) {
            $sheet->setCellValue('A4', "SSS Employer No: {$sssNumber}");
        }
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'sss_report_');
    ...
    public function toPdf(array $data, int $year, ?int $month, ?int $quarter): array
    {
        ...
        $viewData = [
            'company' => [
                ...
                'sss_number' => $tenant?->business_info['sss_number'] ?? null,
            ],
            'report' => [
                'title' => $this->getTitle(),
    ...
    protected function generateFilename(string $extension, int $year, ?int $month, ?int $quarter): string
    {
        ...
        return "sss_{$reportCode}_{$periodPart}.{$extension}";
    }
